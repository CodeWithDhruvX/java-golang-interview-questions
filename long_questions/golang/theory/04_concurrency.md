...
**Your Response:** "Goroutines are functions that execute concurrently with other functions. They are extremely lightweight threads managed by the Go Runtime, not the Operating System. Mechanically, they use an M:N Scheduling model where the runtime multiplexes thousands of goroutines onto a small number of OS threads. They start with a tiny 2KB stack that grows dynamically, allowing you to spawn tens of thousands of them on a single machine without crashing RAM.

In the real world, we use them for everything: handling individual HTTP requests, background cron jobs, or processing messages from a queue. The main trade-off is ensuring they exit - if you spawn a goroutine that blocks forever, you create a 'goroutine leak' that eventually kills your server."
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** How do you start a goroutine?

**Your Response:** "You simply place the keyword `go` in front of any function call: `go process(data)`. This returns control immediately to the next line of code, while the function runs in the background. The arguments to the function are evaluated synchronously at the moment of the call, but the execution of the body happens asynchronously.
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** What is a channel in Go?

**Your Response:** "Channels are the pipes that connect concurrent goroutines. They allow you to send values from one goroutine to another, providing both communication and synchronization. Under the hood, a channel is a thread-safe queue (ring buffer) protected by a mutex. When you send to a channel and nobody is reading, the runtime parks your goroutine efficiently until a receiver arrives.
...
In practice, I use unbuffered channels for strict synchronization (orchestrating timing) and buffered channels for rate limiting or handling bursty traffic. Buffered channels decouple the producer from the consumer, but they can hide bugs if the consumer dies and the producer might fill the buffer and then block, masking the failure for seconds.

---

### How to Explain in Interview (Spoken style format)
**Interviewer:** What is the difference between buffered and unbuffered channels?

**Your Response:** "An unbuffered channel (`make(chan int)`) has zero capacity. It forces a synchronous handoff where the sender blocks until the receiver is ready, effectively guaranteeing that 'make' and 'process' happen at the same time. A buffered channel (`make(chan int, 10)`) has a queue, so the sender can drop off a value and keep going as long as the buffer isn't full.

I use unbuffered channels for strict synchronization like orchestrating timing, and buffered channels for rate limiting or handling bursty traffic. Buffered channels decouple the producer from the consumer, but they can hide bugs if the consumer dies and the producer might fill the buffer and then block, masking the failure for seconds."
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** How do you close a channel?

**Your Response:** "You use the built-in function `close(ch)`. This sends a special signal to all receivers that 'no more data is coming.' Mechanically, closing a channel is a broadcast event that wakes up all parked receivers immediately. Any subsequent receive operation returns the zero value instantly.
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** What happens when you send to a closed channel?

**Your Response:** "It causes a runtime panic. The program crashes immediately. This is a strict design choice by the Go team to prevent ambiguous data flow. A closed channel means 'Data Stream Ended.' Writing to it implies you don't know the stream is dead, which is a logic bug.
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** What is a `sync.WaitGroup`?

**Your Response:** "It's a thread-safe counter used to wait for a collection of goroutines to finish. You call `Add(1)` when you start a task, `Done()` (which decrements the counter) when it finishes, and `Wait()` to block the main thread until the counter hits zero. The most common bug is calling `Add(1)` inside the goroutine instead of before spawning it - this is a race condition.

It's the standard tool for Scatter-Gather patterns. If I need to fetch data from 10 different URLs in parallel, I spawn 10 goroutines, each with a `defer wg.Done()`, and `wg.Wait()` at the end to aggregate the results."
...
In the real world, I use this to protect internal state in structs, usually creating **Thread-Safe** objects. The performance is generally better than channels for simple state updates, but the risk is **Deadlocks**—if you forget to unlock (use `defer Unlock()`!) or if you try to lock the same mutex twice in the same goroutine.

---

### How to Explain in Interview (Spoken style format)
**Interviewer:** How does `sync.Mutex` work?

**Your Response:** "A Mutex provides mutual exclusion - it locks a section of code so only one goroutine can execute it at a time. `Lock()` blocks other goroutines until you call `Unlock()`. It protects shared variables like maps or counters from data corruption during concurrent access.

In the real world, I use this to protect internal state in structs, usually creating Thread-Safe objects. The performance is generally better than channels for simple state updates, but the risk is deadlocks if you forget to unlock. Using `defer Unlock()` is critical to avoid this."
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** What is `sync.Once`?

**Your Response:** "`sync.Once` guarantees that a function executes exactly once, no matter how many goroutines call it simultaneously. It uses an atomic counter and a lock internally to ensure safety. The first goroutine runs the code, the second one calls `recover()` at the end, logs the stack trace, and returns a 500 error to the user. Using panic for regular control flow like 'File Not Found' is considered a major anti-pattern in Go.
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** How do you avoid race conditions?

**Your Response:** "The best way is to avoid shared state entirely. 'Share memory by communicating, don't communicate by sharing memory.' Passing portions of data over channels is safer than locking a global variable.

If you must share state like a cache, use `sync.Mutex` or `atomic` primitives to serialize access. Finally, effective tooling is key. We always run tests with `go test -race`. The Race Detector instruments the code to catch race conditions at runtime. It has high overhead, so we don't run it in production, but it is mandatory in CI/CD."
...
**Your Response:** "The Go Memory Model defines the strict rules of visibility. It answers the question: 'If I write to variable X in Goroutine A, when is Goroutine B guaranteed to see it?' The answer is: Never. The compiler and CPU are free to reorder instructions for performance, so you never know when a write in one goroutine becomes visible to another goroutine unless you use explicit synchronization.

Synchronization primitives like channels and mutexes act as memory barriers. A channel send 'happens-before' the receive completes, guaranteeing that any writes done before the send are visible to the receiver. Without these barriers, you're coding in 'undefined behavior' land where the compiler might cache your write in a register and another goroutine might never see it."
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** How do you use `context.Context` for cancellation?

**Your Response:** "We pass `context.Context` as the first argument to every function in the call stack. To cancel a tree of operations, we create a context with `ctx, cancel := context.WithCancel(parent)`. When we call `cancel()`, the `ctx.Done()` channel closes.

Functions listen to this channel in a select. If the user closes their browser tab, the `request.Context` is cancelled, the `ctx.Done()` channel closes, the select immediately proceeds to the timeout case, and the function returns a 'timeout' error without waiting for the API call to complete. This is standard in HTTP servers - if a user closes their browser tab, the `request.Context` is cancelled, propagating down to the database driver to kill the running SQL query and free up resources."
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** How to pass data between goroutines?

**Your Response:** "We use channels. This is the idiomatic 'Go Way'. By sending a pointer over a channel, you effectively transfer ownership of that data to the receiving goroutine. The sender should stop touching it to avoid race conditions.
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** What is the `runtime.GOMAXPROCS()` function?

**Your Response:** "It limits the number of OS threads that can execute Go code simultaneously. By default, it equals the number of CPU cores. We generally leave this alone, but in containerized environments with CPU quotas, older Go versions sometimes saw the physical host's 64 cores instead of the container's 2-core quota, leading to severe throttling.

In those cases, we used to set `GOMAXPROCS` manually or use the `automaxprocs` library. Setting it to 1 is also a useful debugging trick to serialize execution and reproduce certain race conditions deterministically."

---

## 78. How do you detect deadlocks in Go?
**Answer:**
For global deadlocks where every goroutine is asleep, the Go Runtime detects it automatically and panics with 'fatal error: all goroutines are asleep'. For partial deadlocks where only some threads are stuck, we enable the Block Profiler and inspect the `/debug/pprof/block` endpoint to see stack traces of goroutines that have been waiting on locks for unusually long times.

---

### How to Explain in Interview (Spoken style format)
**Interviewer:** How do you detect deadlocks in Go?

**Your Response:** "For global deadlocks where every goroutine is asleep, the Go Runtime detects it automatically and panics with 'fatal error: all goroutines are asleep'. For partial deadlocks where only some threads are stuck, we enable the Block Profiler and inspect the `/debug/pprof/block` endpoint to see stack traces of goroutines that have been waiting on locks for unusually long times.

This helps us find the 'cycle' where A waits for B, B waits for C, and C waits for A. The block profiler shows us exactly which goroutines are stuck and on which locks, allowing us to identify and fix the deadlock."
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** What are worker pools and how do you implement them?

**Your Response:** "A Worker Pool is a pattern to limit concurrency. Instead of spawning a new goroutine for every single task, you start a fixed number of workers. They all consume from a single shared channel. This creates backpressure - if the workers are busy, the channel fills up and the producer eventually blocks, smoothing out load spikes instead of crashing the server with Out-Of-Memory errors.

It effectively smoothes out the load. If the workers are busy, the channel fills up and the producer blocks, preventing a massive spike in traffic from crashing your server. This is a fundamental pattern for building resilient concurrent systems."
...
### How to Explain in Interview (Spoken style format)
**Interviewer:** How to write concurrent-safe data structures?
