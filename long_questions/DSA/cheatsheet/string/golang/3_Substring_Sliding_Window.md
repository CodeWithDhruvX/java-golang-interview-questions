...
// Brute Force Longest Substring Without Repeating Characters
...
func hasUniqueChars(runes []rune, start, end int) bool {
...
func FindAnagramsBruteForce(s string, p string) []int {
	...
	for i := 0; i <= len(s)-len(p); i++ {
		if areAnagramsSubstring(s[i:i+len(p)], p) {
...
func areAnagramsSubstring(s1, s2 string) bool {
	if len(s1) != len(s2) {
		return false
	}
...
// Brute Force Smallest Window Containing All Characters
...
func MinWindowBruteForce(s string, t string) string {
	...
	for i := 0; i < len(s); i++ {
		for j := i; j < len(s); j++ {
			window := s[i : j+1]
			if containsAllChars(window, tFreq) {
...
func containsAllChars(window string, tFreq map[byte]int) bool {
	windowFreq := make(map[byte]int)
	for i := 0; i < len(window); i++ {
		windowFreq[window[i]]++
	}
	
	for char, count := range tFreq {
